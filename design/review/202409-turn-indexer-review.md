...
> 🔄 迁移中 — 源位置: `docs/design-review-202409-turn-indexer.md`
...
- CLI（`src/cli/index.ts`）：手动调用 `extractMaxTurnNumberFromSession` 提取最大 turn 号再传入
- Agent（`src/agent/agent-runner.ts`）：同样需要自己算
...
`src/sessions/session-repository.ts` 中 `deserializeMessageHistory` 函数先尝试整体校验，失败后 fallback 逐项处理，但非对象类型 entry 被返回 `[]` 丢弃。
...
旧版代码可能在 `SessionMessage` 中使用了 `timestamp` 字段名。当前 schema 要求 `createdAt`。当 `deserializeMessageHistory` 用 `sessionMessageSchema.safeParse` 校验这些旧消息时，因字段名不匹配而失败 → fallback 到 per-item 逻辑 → 非 string entry 被 `return []` 丢弃。
