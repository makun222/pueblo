...
> ✅ 稳定版 — 已包含 `loop-update.md` 的全部后续更新内容（见下文"后续更新"章节）
> 决策日期: 2025-08-07
...
> **以下内容来自 [`loop-update.md`](./loop-update.md)（已合并至此文档）。loop-update.md 现为重定向文件。**

## 后续更新：实时输出流、输入锁定与取消

以下内容是对原始设计的补充和细化，基于实施过程中发现的需求。

### 实时输出流 (OutputPublisher 模式)

LoopJobManager 不再直接将 output buffer 传递给 Renderer，而是通过 `OutputPublisher` 中介：

```
LoopJobManager ──→ OutputPublisher ──→ MonitorWindow (Renderer)
                        │
                        └──→ CLI output (stdout)
```

- `OutputPublisher` 持有可订阅的 `EventEmitter`
- MonitorWindow 在 `did-finish-load` 时订阅
- CLI 模式在启动时订阅
- 输出按 `runId` 分片，确保并发时不乱

#### 关键接口

```typescript
interface OutputPublisher {
  onOutput(runId: string, callback: (chunk: OutputChunk) => void): void;
  push(runId: string, chunk: OutputChunk): void;
...
```
Renderer (Monitor.vue)
    ↕ contextBridge (preload.ts)
    ↕ IPC (ipcRenderer / ipcMain)
    ↕ OutputPublisher (main process)
    ↕ LoopJobManager
```

### 输入锁定 (InputLock)

当 loop 任务运行时，用户输入（文本、粘贴、快捷键）应被锁定，避免干扰。

| 组件 | 锁定措施 |
|------|---------|
| CLI | 禁用 stdin `readline` 接口 |
| Desktop | MonitorWindow 全屏覆盖透明遮罩 + 提示文字 |
| IDE 插件 | 禁用发送按钮，显示 "Loop in progress..." |

#### InputLockManager

```typescript
interface InputLockManager {
...
使用 `AbortController` 链：

```typescript
LoopJobManager.cancel(jobId)
  → LoopRunner.cancel(runId)       // AbortController.abort()
    → LLM 调用中断                  // API stream abort
    → LoopJobManager.onAbort()     // cleanup
      → OutputPublisher.end(runId) // final flush
```

UI 触发途径：
- Desktop: MonitorWindow 上 "取消" 按钮
- CLI: `Ctrl+C` 拦截
- 通知栏: 系统通知可点击取消

### Renderer 修改

MonitorWindow (Renderer) 新增：

1. **markedTextRenderer 覆盖层** — 将 `chunk.type === 'markdown'` 渲染为格式化文本
2. **进度条** — 显示当前 step / total steps
3. **取消按钮** — 固定于右下角
4. **锁定提示** — 当 InputLock 激活时显示

#### 交互流程

```mermaid
graph LR
    A[用户触发 Loop] --> B[LoopJobManager]
    B --> C[创建 OutputPublisher]
    B --> D[锁定输入 - InputLock]
    C --> E[MonitorWindow 订阅]
    B --> F[LoopRunner.run]
    F --> G[LLM 调用]
    G --> H[输出流推送]
    H --> E
    G --> I{是否完成?}
    I -->|是| J[OutputPublisher.end]
    I -->|否| K[继续下一步]
    K --> G
    J --> L[解锁输入]
    L --> M[完成通知]
...
### 变更清单 (Change Inventory)

| 组件 | 文件 | 变更类型 | 说明 |
|------|------|---------|------|
| Core | `src/loop/output-publisher.ts` | 新增 | OutputPublisher 类 |
| Core | `src/loop/input-lock.ts` | 新增 | InputLockManager 类 |
| Core | `src/loop/types.ts` | 修改 | 新增 `OutputChunk`、`InputLockState` 类型 |
| Manager | `src/loop/loop-job-manager.ts` | 修改 | 集成 OutputPublisher、InputLock、Cancel |
| Runner | `src/loop/loop-runner.ts` | 修改 | 支持 AbortController |
| Desktop | `src/desktop/main/ipc.ts` | 修改 | 新增 loop:subscribe、loop:cancel IPC |
| Desktop | `src/desktop/renderer/preload.ts` | 修改 | 暴露 subscribe/cancel API |
| Desktop | `src/desktop/renderer/Monitor.vue` | 修改 | 集成 markedTextRenderer、进度条、取消按钮 |
| CLI | `src/cli/loop.ts` | 修改 | 集成 OutputPublisher、InputLock |
| CLI | `src/cli/index.ts` | 修改 | Ctrl+C 拦截 |
...
| 任务 | 文件 | 工时 |
|------|------|------|
| 创建 OutputPublisher | `src/loop/output-publisher.ts` | 2h |
| 创建 InputLockManager | `src/loop/input-lock.ts` | 1h |
| 更新 types.ts | `src/loop/types.ts` | 0.5h |
| 修改 LoopJobManager | `src/loop/loop-job-manager.ts` | 3h |
| 修改 LoopRunner | `src/loop/loop-runner.ts` | 2h |
| 单元测试 | `src/loop/__tests__/` | 3h |
| **小计** | | **11.5h** |
...
| 任务 | 文件 | 工时 |
|------|------|------|
| IPC 新通路 | `src/desktop/main/ipc.ts` | 1.5h |
| preload 暴露 | `src/desktop/renderer/preload.ts` | 1h |
| Monitor.vue 更新 | `src/desktop/renderer/Monitor.vue` | 4h |
| CLI 集成 | `src/cli/loop.ts` + `index.ts` | 2h |
| 集成测试 | `tests/` | 3h |
| **小计** | | **11.5h** |
| **总计** | | **23h** |
...
| ADR | 决策 | 理由 |
|-----|------|------|
| ADR-1 | OutputPublisher 作为独立模块而非 LoopJobManager 方法 | 解耦，CLI 和 Desktop 可复用 |
| ADR-2 | 使用 EventEmitter 而非 RxJS | 依赖轻量，足够满足当前需求 |
| ADR-3 | AbortController 链而非自定义信号 | 原生 API，与 fetch/stream 天然兼容 |
| ADR-4 | InputLock 全局单例而非实例传递 | 简化状态管理，避免多层透传 |

### 风险与缓解

| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|---------|
| 并发 loop 任务输出混乱 | 高 | 中 | OutputPublisher 按 runId 分片 |
| 取消后资源泄漏 | 高 | 低 | AbortController 链 + finally 清理 |
| InputLock 死锁 | 中 | 低 | 超时自动解锁（30s） |
| Renderer 性能（大量输出） | 中 | 中 | 虚拟滚动 + 输出截断（保留最近 1000 行） |
