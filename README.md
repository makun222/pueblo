...
会话层选中：指 session.selectedMemoryIds，也就是这条 session 认为“挂在自己身上”的记忆集合，来源于手动选择、批量设置、跨 session 导入，以及后台自动加入的 summary memory。

上下文层选中：指本轮真正进入 prompt 的 resultItems。它不是简单等于 selectedMemoryIds，而是要再经过一次 Pepe 的相关性筛选，最后由 task-message-builder.ts组装成 Relevant result items 这个 system block 注入模型上下文。
session 里维护“候选记忆集合”，真正进上下文前还会再经过一次相关性筛选。

#### Pepe

Pepe是Pueblo的内置Memory管理系统，提供了一个统一的接口来存储和检索与会话相关的信息。Pepe支持多种类型的Memory。

### Talkto

实现了agent A talkto agent B的功能，A和B可以是两个agent实例。发起方A会将起始消息作为input发送给B，接收B处理后的响应。如此往复，最多50轮。
未来这个功能可以用来实现agent之间的协作的基础，例如一个agent专门负责代码分析，另一个agent专门负责代码生成，它们可以通过talkto互相调用对方的能力来完成更复杂的任务。

### Agent

Agent是一个实例，运行时只能选择一个模板。单个模板每次只能运行一个实例。如果同时开启两个Code Master Agent，目前memory的持久化会混乱，当然这也可以简单地通过不同的session来规避。
初始的想法是，Agent如同一个人，按照初始模板存在，在经历了不同事件后，会形成不同的个性和能力倾向，这些都记录在Memory里。
Agent的模板则是这个人的职业训练背景，决定了这个人最初拥有什么样的能力倾向和思维方式。
在事件中形成的个性和能力，会体现在Skill的积累、Workflow的构建，其区别大致是：Skill是具体能力或流程，以提示词+脚本的方式存在；workflow则是规则流程的体现，以硬编码方式存在。
将来，在经历某件事或多件事后，Agent会修改自身代码，当然，主框架不能变，而是将workflow代码重写或添加新workflow，实现进化。甚至，也可以修改agent.md。
...
如果你在同一台机器上既会运行 Node 侧测试，也会运行 Electron 桌面，需要注意 `better-sqlite3` 是原生模块，必须和当前运行时 ABI 一致：

- 运行桌面前，执行：`npm run rebuild:electron-native`
- 运行依赖 SQLite 的 Node 测试前，执行：`npm run rebuild:node-native`

原因：

- Electron 37 需要 Electron 自己的原生 ABI
- Node.js 22 需要 Node 22 的原生 ABI
- 两者不能共用同一份刚编译好的 `better-sqlite3.node`

典型现象：

- 桌面报 `NODE_MODULE_VERSION 141 ... requires 136`，说明你刚把模块编译成 Node 版本了，此时执行 `npm run rebuild:electron-native`
- Node 测试报 `NODE_MODULE_VERSION 136 ... requires 141`，说明你刚把模块编译成 Electron 版本了，此时执行 `npm run rebuild:node-native`
...
Pueblo 当前对复杂任务提供显式 workflow 编排层，首个 workflow 类型是 `pueblo-plan`。

- 触发方式有三种：显式 `/workflow <goal>`、输入命中 workflow 关键字、或任务复杂度超过单轮 step budget。
- workflow 启动后会先在工作区 `.plans/` 下创建运行态 `.plan.md`，路径形如 `.plans/<workflowId>/<slug>.plan.md`。
- `.plans/` 中的 plan 是内部执行台本；只有 workflow 完成后，最终 `.plan.md` 才会导出到目标工程目录。
- 活跃 `plan` 和当前轮 `todo` 会通过 workflow context 固定注入，不依赖 Pepe 排名是否命中。
- `selectedMemoryIds` 仍可保留 `plan/todo` 作为候选或可视化元数据，但不再决定它们是否进入 prompt。

推荐操作顺序：

1. 用 `/workflow <goal>` 显式接管复杂任务，或直接输入复杂文本任务让系统自动 handoff。
2. 通过连续 `/task-run <next step>` 推进当前轮工作，查看结果中的 `Workflow` / `Workflow Export` block。
3. 如需检查执行态 plan，直接查看 `.plans/` 下对应 workflow 目录，而不是依赖普通 memory 搜索结果。
4. workflow 完成后，再检查目标工程目录中的最终 `.plan.md` 交付物。
...
- Workflow 规格：`specs/002-workflow-orchestration/spec.md`
- Workflow 计划：`specs/002-workflow-orchestration/plan.md`
- Workflow 快速验证：`specs/002-workflow-orchestration/quickstart.md`
